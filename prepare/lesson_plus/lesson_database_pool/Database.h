...
public:
    ...
    bool registerUser(const std::string& username, const std::string& password) {
        ...
        if (!stmt) {
            // LOG_INFO("Failed to prepare registration SQL"); 
            return false;
        }
        ...
        if (sqlite3_step(stmt) != SQLITE_DONE) {
            ...
            sqlite3_reset(stmt); // 重置stmt以便下次重用
            return false;
        }

        sqlite3_reset(stmt);
        return true; 
        // 注意：不需要手动 returnConnection(db)，conn对象析构时自动处理
    }
    ...
    bool loginUser(const std::string& username, const std::string& password) {
        ...
        if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
            // LOG_INFO("Failed to prepare login SQL");
            return false;
        }
        ...
        std::string dbStoredHash = stored_pass ? 
                                   reinterpret_cast<const char*>(stored_pass) : "";
