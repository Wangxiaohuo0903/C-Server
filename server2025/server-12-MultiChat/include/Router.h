...
inline void Router::setupSessionRoutes(SessionManager& sm, ModelManager& mm) {
    ...
    sessionChatHandler = [&sm, &mm](const HttpRequest& r) {
        try {
            ...
            // 3. 调用模型生成回复
            std::string full_prompt = context + "User: " + user_message + "\nAssistant:";
            std::string assistant_reply = mm.infer(session_id, full_prompt, maxTokens, temperature);
...
inline void Router::setupStaticPages() {
    ...
    addStatic("/multichat.html", "../UI/multichat.html");  // Server-12新增：多对话界面
    addStatic("/style.css",     "../UI/style.css");
    addStatic("/multichat.css", "../UI/multichat.css");    // Server-12新增：多对话样式
