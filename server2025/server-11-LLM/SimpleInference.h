...
public:
    ...
    std::string generate(const std::string& prompt, int max_tokens = 64) {
        ...
        // 4. 生成tokens（传递prompt的token数量作为起始位置）
...
private:
     ...
     * @param n_past_init 已处理的token数量（prompt的长度）
     */
    std::string generate_tokens(llama_context* ctx, const llama_vocab* vocab, int max_tokens, int n_past_init) {
        std::string output;
