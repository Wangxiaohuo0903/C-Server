...
#include <sstream>
#include <algorithm>
#include <cstdio>  // Server-14: for printf
#include <iostream>  // Server-14: for debug cout
...
public:
    ...
    std::unordered_map<std::string,std::string> parseJson() const {
        ...
        // Server-14: 调试 - 打印原始 body（检查 UTF-8 编码）
        std::cout << "[HttpRequest::parseJson] body length: " << body.length() << "\n";
        std::cout << "[HttpRequest::parseJson] body: " << body << "\n";
        std::cout << "[HttpRequest::parseJson] body hex (first 150 bytes): ";
        for (size_t idx = 0; idx < std::min(body.length(), size_t(150)); ++idx) {
            printf("%02x ", (unsigned char)body[idx]);
        }
        std::cout << "\n";
        ...
        while (i < body.size()) {
            ...
            if (body[i] == '"') {  // 处理字符串值
                ...
                // Server-14: 调试 - 打印解析出的 key/value
                std::cout << "[HttpRequest::parseJson] Parsed: key=\"" << key << "\", val=\"" << val << "\" (val_hex: ";
                for (size_t idx = 0; idx < std::min(val.length(), size_t(50)); ++idx) {
                    printf("%02x ", (unsigned char)val[idx]);
                }
                std::cout << ")\n";
            } else {  // 处理数字、布尔、null
                size_t v2 = i;
    ...
    // Server-14: Get header value by key (returns empty string if not found)
