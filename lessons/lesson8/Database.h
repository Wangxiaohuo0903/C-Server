...
bool registerUser(const std::string& username, const std::string& password) {
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        ...
        LOG_INFO("Registration failed for user: " + username);
    ...
    sqlite3_finalize(stmt);
    // 记录用户注册成功的日志
    LOG_INFO("User registered: " + username + " with password: " + password);
    return true;
}
...
bool loginUser(const std::string& username, const std::string& password) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
        // 如果准备SQL语句失败，记录日志并返回false
        LOG_INFO("Failed to prepare login SQL for user: " + username);
        return false;
    }
    ...
    if (stored_password == nullptr || password != password_str) {
        ...
        LOG_INFO("Login failed for user: " + username + " password:" + password + " stored password is " + password_str);
        return false;
    }
