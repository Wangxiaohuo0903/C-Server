...
public:
    ...
    std::string toString() const {
        ...
        // 如果响应体已被压缩，确保已经设置了 Content-Encoding 头
    ...
    void compressBody() {
        ...
        uLongf compressedDataSize = compressBound(body.length());
        std::vector<Bytef> compressedData(compressedDataSize);
        
        if (compress(compressedData.data(), &compressedDataSize, 
            reinterpret_cast<const Bytef*>(body.data()), body.length()) == Z_OK) {
            body = std::string(reinterpret_cast<char*>(compressedData.data()), compressedDataSize);
            setHeader("Content-Encoding", "gzip"); // 设置响应头以指示内容已被GZIP压缩
        } else {
            // 压缩失败，可以记录日志或处理错误
        }
    }
